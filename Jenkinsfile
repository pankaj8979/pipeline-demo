...
pipeline {
    ...
    stage("Workspace cleanup"){
        ...
        stage('Checkout') {
            steps {
	    script{
	        clone("https://github.com/pankaj8979/pipeline-demo.git", "main")
		}
	...
	stage("Trivy: Filesystem scan"){
            steps{
                script{
                    sh """
		    trivy image --no-progress --severity HIGH,CRITICAL --format json --output /tmp/trivy-report.json my-node-app:latest
                    """
	       }
            }
        }
        ...
        stage('Example Stage') {
            steps {
                script {
                    ...
                    echo "Pankaj Jain is hot suvrata baby come fast naaaa!"
